...
class TemplateFileService:
    ...
    def replace_template_file(
        ...
        if record is None:
            raise AppError("TEMPLATE_NOT_FOUND", f"Template not found: {template_id}", 404, {"template_id": template_id})
        if Path(original_filename).suffix.lower() != ".docx":
            raise AppError("UPLOAD_FILE_INVALID", "Only .docx template files are supported", 400)

        self._validate_docx(file_bytes)
        temp_path = self._write_temp_template_file(template_id, file_bytes)
        try:
            try:
        ...
        updated_record = self.repository.get_template_file(template_id)
    ...
    def _validate_template_bindings(self, template_path: Path, requirements: TemplateRequirements) -> None:
        ...
        references, _original_by_canonical, missing_variables = analyze_template_variables(
            template_path,
            requirements.fields,
        )
        ...
        for reference in references:
            ...
            if reference.table_name == "data":
                errors.append(
                    {
                        "code": "template_variable_legacy_data_alias",
                        "message": f"Template variable must use table.field instead of data.field: {original_path}",
                        "table_name": reference.table_name,
                        "field_name": reference.field_name,
                    }
                )
                continue
            if reference.table_name not in allowed_tables:
                errors.append(
                    {
                        "code": "template_variable_table_not_configured",
                        "message": f"Template references table not configured for this template: {reference.table_name}",
                        "table_name": reference.table_name,
                        "field_name": reference.field_name,
                    }
                )
                continue
            if reference.field_name not in fields_by_table.get(reference.table_name, set()):
                errors.append(
                    {
                        "code": "missing_field",
                        "message": f"Template references field not found in schema: {reference.variable_path}",
                        "table_name": reference.table_name,
                        "field_name": reference.field_name,
                    }
                )
    ...
    def _write_temp_template_file(self, template_id: int, file_bytes: bytes) -> Path:
        self.settings.temp_dir.mkdir(parents=True, exist_ok=True)
        temp_path = ensure_path_under_base(
            self.settings.temp_dir / f"template_replace_{template_id}_{uuid4().hex}.docx",
