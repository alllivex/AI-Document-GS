<template>
  <el-dialog :model-value="modelValue" title="导入实体Schema" width="760px" @update:model-value="handleVisibleChange">
    <div class="import-dialog">
      ...
      >
        <el-button>选择 entity_schema.xlsx</el-button>
        <template #tip>
      ...
      <template v-if="preview">
        <el-descriptions :column="3" border>
          ...
          <el-descriptions-item label="跳过">{{ preview.summary.skip_count }}</el-descriptions-item>
...
const preview = ref<EntitySchemaImportPreview | null>(null)
...
async function previewImport() {
  ...
  errorMessage.value = ''
  try {
    preview.value = await previewEntitySchemaImport(selectedFile.value)
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : '实体Schema预校验失败'
  } finally {
...
async function commitImport() {
  ...
  errorMessage.value = ''
  try {
    await commitEntitySchemaImport(preview.value.import_id)
    ElMessage.success('实体Schema导入成功')
    emit('imported')
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : '实体Schema导入失败'
  } finally {
...
function levelText(level: EntitySchemaImportLevel) {
  const map: Record<EntitySchemaImportLevel, string> = {
    info: '信息',
    warning: '警告',
    error: '错误',
  }
